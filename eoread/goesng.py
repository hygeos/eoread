...
def Level1_GOESNG(file_1km,
    ...
    if isinstance(chunksize, dict):
        chunk_x = chunksize['x']
        chunk_y = chunksize['y']
        assert (chunk_x < 0) or (chunk_x % 4 == 0)
        assert (chunk_y < 0) or (chunk_y % 4 == 0)
        chunksize_1km = {
            'nx1km': chunk_x,
            'ny1km': chunk_y,
            'nx4km': chunk_x//4 if (chunk_x > 0) else chunk_x,
            'ny4km': chunk_y//4 if (chunk_y > 0) else chunk_y,
            'nx500m': chunk_x*2 if (chunk_x > 0) else chunk_x,
            'ny500m': chunk_y*2 if (chunk_y > 0) else chunk_y,
        }
        chunksize_500m = chunksize_1km
        chunks_latlon = {
            'Nlin': chunk_y,
            'Ncol': chunk_x,
        }
        ...
        chunksize_1km = chunksize
        chunksize_500m = chunksize*2
    ...
    # Load 500m data
    ds_500m = xr.open_dataset(file_500m, chunks=chunksize_500m)
    # downsample
    arr_resampled = 0.
    for i in [0, 1]:
        for j in [0, 1]:
    ...
    if convert_auxfile:
        ...
        aux = xr.open_dataset(ncfile, chunks=chunks_latlon)
    else:
    ...
    if cloudmask:
        ...
        cm = xr.open_dataset(cmaskfile, chunks=chunksize_cm)

        cma = xr.DataArray(
            da.repeat(da.repeat(cm.cma.data, 2, axis=0), 2, axis=1),
            dims=(naming.rows, naming.columns),
        )
